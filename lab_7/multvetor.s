...
MultiplicaVetor:
    ...
    mv   s0, a0 #qnt elementos do vetor
    mv   s1, a1 #vetor
    mv   s2, a2 #por qnt tô multiplicando

for:
    beq  s0, zero, fim #qnd ja tiver percorrido tds elementos
    lw   a0, 0(s1) #coloca no a0 o num
    #mv   a1, s2 #coloca o fator q vou multiplicar
    #call Multiplica
    mul a0, a0, s2
    sw   a0, 0(s1) #escreve a resp no vetor
    addi s1, s1, 4
    addi s0, s0, -1
    j    for
